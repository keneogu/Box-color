...
const Header = ({clickHandle}) => {
	return (
		<div className="header d-flex justify-content-between p-2 bg-secondary text-white">
			...
			<li className='head-li pr-5 text-white w-100' onClick={()=>{clickHandle('blue')}}>Blue</li>
			<li className='head-li pr-5 text-white w-100' onClick={()=>{clickHandle('red')}}>Red</li>
			<li className='head-li pr-3 text-white w-100' onClick={()=>{clickHandle('brown')}}>Brown</li>
			</div>
		</div>
	)
}
